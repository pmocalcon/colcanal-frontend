...
import { Home, ArrowLeft, Search, Eye, CheckCircle, XCircle, ClipboardCheck } from 'lucide-react';
...
export default function RevisarLevantamientosPage() {
  ...
  const loadSurveysForReview = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await surveysService.getSurveysForReview();
      setSurveys(data.data || []);
      setFilteredSurveys(data.data || []);
    } catch (err: any) {
      console.error('Error loading surveys for review:', err);
      setError(err.response?.data?.message || 'Error al cargar los levantamientos pendientes');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex-grow text-center">
              ...
              <p className="text-xs md:text-sm text-[hsl(var(--canalco-neutral-600))]">
                {filteredSurveys.length} levantamientos pendientes de revisión
              </p>
            </div>
      ...
      <main className="flex-grow max-w-7xl mx-auto px-6 py-8 w-full">
        ...
        <div className="mb-6 flex gap-4">
          <div className="relative flex-grow max-w-md">
            ...
            <Input
              type="text"
              placeholder="Buscar por código de obra, nombre..."
              value={searchTerm}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-md border border-[hsl(var(--canalco-neutral-300))] p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-[hsl(var(--canalco-neutral-700))] mb-2">
              {searchTerm ? 'No se encontraron levantamientos' : 'No hay levantamientos pendientes'}
            </h3>
            <p className="text-[hsl(var(--canalco-neutral-500))]">
              {searchTerm
                ? 'Intenta con otros términos de búsqueda'
                : 'Todos los levantamientos han sido revisados'}
            </p>
          </div>
        ) : (
          /* Surveys Table */
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => alert('Aprobar - Próximamente')}
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => alert('Rechazar - Próximamente')}
